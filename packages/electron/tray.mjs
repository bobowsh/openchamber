...
const computeTooltip = (counts, sessionCount) => {
  if (sessionCount === 0) return 'LinkCode — no active sessions';
  const bits = [];
  ...
  return `LinkCode — ${sessionCount} session${sessionCount === 1 ? '' : 's'}${suffix}`;
};
...
export const createTrayController = ({ idleIconPath, unseenIconPath, breathIconPaths, statusIconPaths, onAction }) => {
  ...
  const buildMenu = (snapshot) => {
    ...
    template.push(
      ...
      { label: 'Show LinkCode', click: () => onAction({ type: 'show-main-window' }) },
      { type: 'separator' },
      { label: 'Quit LinkCode', click: () => onAction({ type: 'quit' }) },
    );
